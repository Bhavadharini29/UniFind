      ...
      <div className=" h-full px-4 md:px-10 pt-28 pb-6 w-full ">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {((filteredSearchItems && filteredSearchItems.length > 0) || (filteredSearchItems === null && searchCategory === 0)
            ...
            <div
              key={item.id}
              className={`w-auto px-5 py-3 bg-white rounded-2xl flex flex-col justify-start items-start gap-3 overflow-hidden shadow-lg transition duration-300 transform hover:scale-105 ${index > 0 ? 'delay-100' : ''
                }`}
            >
              <img
                src={item.image}
